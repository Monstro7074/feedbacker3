...
function extractTagsAndSummary(text) {
  ...
  // 1) Summary: первые предложения до 200 символов
  ...
  // 2) Доменные теги + синонимы → канонический тег
  const canonicalRules = [
    { re: /\b(сидит|сидят|посадк\w*|садитс\w*)\b/giu, tag: 'посадка' },
    { re: /\b(размер\w*|маломер\w*|большемер\w*)\b/giu, tag: 'размер' },
    { re: /\b(качест\w*|брак\w*|шв\w*|нитк\w*)\b/giu, tag: 'качество' },
    { re: /\b(цен\w*|стоимост\w*|дорог\w*|дешев\w*)\b/giu, tag: 'цена' },
    { re: /\b(доставк\w*|курьер\w*|срок\w*)\b/giu, tag: 'доставка' },
    { re: /\b(персонал\w*|сотрудник\w*|продавц\w*|консультант\w*|груб\w*)\b/giu, tag: 'персонал' },
  ...
  // 3) Частотные слова (фоллбек), но с расширенным стоп-листом
  const stop = new Set([
    ...
    'очень','реально','правда','прям','просто','ещё','там','здесь','вот','сами','само','сама','сам',
  ...
  const top = [...freq.entries()]
    .filter(([, c]) => c >= 2)        // берём только повторяющиеся слова, чтобы убрать случайные
    .sort((a,b)=>b[1]-a[1])
    .slice(0, 5)
    .map(x => x[0]);

  // 4) Смешиваем: сначала канонические (важнее), потом фоллбек, максимум 3
  const seen = new Set();
  const tags = [];
  for (const w of [...canonical, ...top]) {
