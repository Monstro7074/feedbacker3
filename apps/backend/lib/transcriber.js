...
export async function transcribeAudio(
  ...
  try {
    ...
    if (!ok && typeof data?.error === 'string' &&
        /not available in this language|sentiment_analysis/i.test(data.error)) {
    ...
    while (true) {
      ...
      if (js.status === 'error') throw new Error(`Ошибка транскрипции: ${js.error}`);
    }
  } catch (err) {
    console.error('❌ Ошибка в transcribeAudio:', err.message);
